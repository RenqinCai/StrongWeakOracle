"""
offline learning via considering weak labels as data poison and remove poisoned data to train a better classifier, compare distance of features with sphere threshold
"""
...
class active_learning:
	...
	def generateCleanData(self, train, sphereDisThreshold):
		...
		slabDisThreshold = 0.0
		sphereDisThreshold = sphereDisThreshold
	...
	def run_CV(self):
		...
		sphereThresholdList = np.arange(7.0, 15.0, 0.5)
		for sphereThreshold in sphereThresholdList:
			print("*************************sphereThreshold: %f****************"%sphereThreshold)
			cvIter = 0
			# random.seed(3)
			...
			for foldIndex in range(foldNum):
				...
				cleanFeatureTrain, cleanLabelTrain = self.generateCleanData(train_sampled, sphereThreshold)

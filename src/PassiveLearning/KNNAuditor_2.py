	...
	def run_CV(self):
		...
		for KNNNeighbors in KNNNeighborsList:
			...
			for foldIndex in range(foldNum):
				...
				KNNClf.fit(self.target_fn[train][:, adjFeatureList], self.auditor_label[train])

				predAuditorLabels = KNNClf.predict(self.target_fn[test][:,adjFeatureList])
				...
				"""
				fit a knn clf on weak oracle's labels and use the majority vote of weak labels as the prediction of the strong label. Compare the predicted strong label with the weak label as the auditor prediction. Compare the auditor prediction with the true auditor label as the accuracy
				"""
				...
				predTargetLabels = KNNClf.predict(self.target_fn[test][:, adjFeatureList])
				neighborsIDList = KNNClf.kneighbors(self.target_fn[test][:, adjFeatureList])


				
				for ID in test:
					print(ID, neighborsIDList[])

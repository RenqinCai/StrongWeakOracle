"""
use the weak oracle's labels as jump start to initialize the active learner 
query instance according to the uncertainty of the active learner
"""
...
class active_learning:
	...
	def select_example(self, unlabeled_list):
		...
		for unlabeledIdIndex in range(unlabeledIdNum):
			...
			unlabeledDistance = self.m_clf.decision_function(self.fn[unlabeledId].reshape(1, -1))
	...
	def run_CV(self):
		...
		for foldIndex in range(foldNum):
			...
			while queryIter < rounds:
				...
				fn_train_iter = self.fn[unlabeledExList]
				label_train_iter = self.transferLabel[unlabeledExList]

				if len(labeledExList) > 0:
					fn_train_iter_strong = self.fn[labeledExList]
					label_train_iter_strong = self.label[labeledExList]

				fn_train_iter = np.vstack((fn_train_iter_strong, fn_train_iter))
				label_train_iter = np.hstack((label_train_iter_strong, label_train_iter))

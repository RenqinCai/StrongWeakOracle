"""
print results to excel to debug auditor offline
"""
...
class active_learning:
	...
	def run_CV(self):
		...
		vocFile = "../../dataset/processed_acl/processedBooksElectronics/books_electronicsvocab"
		vocF = open(vocFile)
		id2WordMap = {}
		for line in vocF:
			wordAndID = line.strip().split(":")
			word = wordAndID[0]
			wordID = int(wordAndID[1])

			id2WordMap.setdefault(wordID, word)

		debugFile = "auditor_wrongDebug.txt"
		printWrongFeature(id2WordMap, debugFile, self.transferLabel[test], self.targetLabel[test], label_test, label_preds, self.fn[test])

		debugFile = "auditor_correctDebug.txt"
		printCorrectFeature(id2WordMap, debugFile, self.transferLabel[test], self.targetLabel[test], label_test, label_preds, self.fn[test])
		# debugExl = "auditor_rfc.xlsx"
		# savePredLabel(debugExl, self.transferLabel[test], self.targetLabel[test], label_test, label_preds)
...
def printWrongFeature(id2WordMap, debugFile, predictLabelList, trueLabelList, label_test, label_preds, feature):
	f = open(debugFile, "w")

	labelNum = len(label_test)
	for labelIndex in range(labelNum):
		if label_test[labelIndex] != label_preds[labelIndex]:
			featureInstance = feature[labelIndex]
			featureNum = len(featureInstance)
			f.write(str(predictLabelList[labelIndex])+"\t"+str(trueLabelList[labelIndex])+"\t"+str(label_preds[labelIndex])+"\t"+str(label_test[labelIndex])+"\t")
			for featureIndex in range(featureNum):
				featureVal = featureInstance[featureIndex]
				featureName = id2WordMap[featureIndex]
				if featureVal == 1:
					f.write(featureName+"\t")
			f.write("\n")
...
def printCorrectFeature(id2WordMap, debugFile, predictLabelList, trueLabelList, label_test, label_preds, feature):
	f = open(debugFile, "w")
	...
	for labelIndex in range(labelNum):
		if label_test[labelIndex] == label_preds[labelIndex]:
			featureInstance = feature[labelIndex]
			featureNum = len(featureInstance)
			f.write(str(predictLabelList[labelIndex])+"\t"+str(trueLabelList[labelIndex])+"\t"+str(label_test[labelIndex])+"\t")
			for featureIndex in range(featureNum):
				featureVal = featureInstance[featureIndex]
				featureName = id2WordMap[featureIndex]
				if featureVal == 1:
...
def savePredLabel(labelFile, predictLabelList, trueLabelList, auditorLabelList, predAuditorLabelList):
	...
	worksheet.write(1, 1,"predictLabel")
	worksheet.write(1, 2, "trueLabel")
	worksheet.write(1, 3, "auditorLabel")
	worksheet.write(1, 4, "predAuditorLabel")

	for labelIndex in range(labelNum):
		colIndex = 1
		worksheet.write(labelIndex+2, colIndex, predictLabelList[labelIndex])
		colIndex += 1
		worksheet.write(labelIndex+2, colIndex, trueLabelList[labelIndex])
		colIndex += 1
		worksheet.write(labelIndex+2, colIndex, auditorLabelList[labelIndex])
		colIndex += 1
		worksheet.write(labelIndex+2, colIndex, predAuditorLabelList[labelIndex])

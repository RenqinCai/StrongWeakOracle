"""
adv active learning offline for auditor by considering the predicted label with multiple auditors
multiple auditors maybe helpful to improve the convergence rate of auditors
"""
...
class active_learning:
	...
	def __init__(self, fold, rounds, fn, tranfersLabel, label):
		...
		##[featureIndexList for an auditor]
		self.m_featureIndexMatrix = []

		self.m_auditorNum = 10
		##[auditor]
		self.m_auditorList = []
	...
	def initMultipleAuditor(self):
		featureNum = len(self.fn[0])
		featureIndexList = [i for i in range(featureNum)]
		self.m_featureIndexMatrix = [[] for i in range(self.m_auditorNum)]
		self.m_auditorList = [None for i in range(self.m_auditorNum)]

		for auditorIndex in range(self.m_auditorNum):
			self.m_auditorList[auditorIndex] = LR(random_state=3)
			self.m_featureIndexMatrix[auditorIndex] = random.sample(featureIndexList, featureNum-1000)

	def trainMultipleAuditor(self, fn_train, label_train):
		for auditorIndex in range(self.m_auditorNum):
			print("auditorIndex", auditorIndex)
			# print(self.m_featureIndexMatrix[auditorIndex])

			self.m_auditorList[auditorIndex].fit(fn_train[:, self.m_featureIndexMatrix[auditorIndex]], label_train)


	def testMultipleAuditor(self, fn_test, label_test):
		predLabels = []

		for auditorIndex in range(self.m_auditorNum):
			predLabelAuditor = self.m_auditorList[auditorIndex].predict(fn_test[:, self.m_featureIndexMatrix[auditorIndex]])
			accAuditor = accuracy_score(label_test, predLabelAuditor)
			print('ACC per auditor', accAuditor)

			predLabels.append(predLabelAuditor)

		predLabels = np.array(predLabels)

		predLabels = stats.mode(predLabels)[0][0]
		print("predLabels", predLabels)
		acc = accuracy_score(label_test, predLabels)
	...
	def run_CV(self):
		...
		for foldIndex in range(foldNum):
			...
			self.trainMultipleAuditor(fn_train, label_train)
			acc = self.testMultipleAuditor(fn_test, label_test)
...
def readTransferLabel(transferLabelFile):
	...
	return transferLabelList, auditorLabelList

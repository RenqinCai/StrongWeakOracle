...
def logistic_pred(weights, inputs):
	...
	outputs1 = np.matmul(inputs, weights[:weightNum/2])
	# print("outputs step1", outputs1)

	outputs2 = np.matmul(inputs, weights[weightNum/2:])
...
class active_learning:
	...
	def initAuditor(self, featureDim):
		...
		self.m_auditorWeight = np.random.uniform(-0.1, 0.1, size=2*featureDim)
	...
	def trainAuditor(self, fn_train, label_train):
		...
		for i in range(epoch):
			self.m_auditorWeight -= training_gradient_fun(self.m_auditorWeight, fn_train, label_train)

			print("weights", self.m_auditorWeight)
			# label_prob[labelProbIndex] = label_prob[labelProbIndex]+1e-30
			for labelProbIndex in range(len(self.m_auditorWeight)):
				labelProbVal = self.m_auditorWeight[labelProbIndex]
				if labelProbVal == 0.0:
					print("labelProb zero err", labelProbIndex, labelProbVal)

	def predAuditor(self, fn_test, label_test):
		pred_test = logistic_pred(self.m_auditorWeight, fn_test)

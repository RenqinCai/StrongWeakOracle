"""
active learning with a strong oracle and weak labels are given. use mislabel to select instance for updating
"""
...
class _ActiveClf:
	...
	def initActiveClf(self, initialSampleList, train, test):
		...
		if self.m_multipleClass:
			self.m_weakClf = LR(multi_class="multinomial", solver='lbfgs',random_state=3,  fit_intercept=False)
		else:
	...
	def select_example(self, corpusObj):
		...
		for unlabeledIdIndex in range(unlabeledIdNum):
			...
			# score = self.getMislabeledProb(corpusObj, unlabeledId)
	...
	def getMislabeledProb(self, corpusObj, unlabeledId):
		labelPredictProb = self.m_activeClf.predict_proba(corpusObj.m_feature[unlabeledId].reshape(1, -1))[0]
		# print(labelPredictProb)
		transferLabel = corpusObj.m_transferLabel[unlabeledId]

		if labelPredictProb[int(transferLabel)] > 0.5:
			return 0
		else:
			return np.abs(labelPredictProb[0]-labelPredictProb[1])

	def getStrongWeakDiffProb(self, corpusObj, unlabeledId):
		labelPredictProb = self.m_activeClf.predict_proba(corpusObj.m_feature[unlabeledId].reshape(1, -1))[0]
		print("labelPredictProb", labelPredictProb)
		weakLabelPredictProb = self.m_weakClf.predict_proba(corpusObj.m_feature[unlabeledId].reshape(1, -1))[0]
		print("weakLabelPredictProb", weakLabelPredictProb)

		probDiff = np.abs(labelPredictProb[1]-weakLabelPredictProb[1])
	...
	def generateCleanDataByWeakClf(self, corpusObj):
		...
		for trainIndex in range(sampledTrainNum):
			...
			if transferLabel == 1.0:
				...
				predictAuditorLabel = 0.0
				# print(self.m_activeClf.predict_proba(featureTrain.reshape(1, -1)))
				if self.m_activeClf.predict_proba(featureTrain.reshape(1, -1))[0][0] >= probThreshold:
					predictAuditorLabel = 1.0
	...
	def getCentroids(self, corpusObj):
		...
		for trainIndex in range(sampledTrainNum):
			...
			if trueLabel == 1:
				posExpectedFeatureTrain += featureTrain
				posLabelNum += 1.0
			else:
		...
		return posExpectedFeatureTrain, negExpectedFeatureTrain
	...
	def activeTrainClf(self, corpusObj):
		...
		weak_feature_train_iter = np.vstack((corpusObj.m_feature[self.m_unlabeledIDList], corpusObj.m_feature[self.m_labeledIDList]))
		weak_label_train_iter = np.hstack((corpusObj.m_transferLabel[self.m_unlabeledIDList], corpusObj.m_label[self.m_labeledIDList]))

		self.m_activeClf.fit(weak_feature_train_iter, weak_label_train_iter)
		...
		# posExpectedFeatureTrain, negExpectedFeatureTrain = self.getCentroids(corpusObj)
		...
		while strongLabelNumIter < self.m_strongLabelNumThresh:
			...
			if self.m_cleanStrategy == "slab":
				cleanFeatureTrain, cleanLabelTrain = self.generateCleanDataBySlab(corpusObj, self.m_slabThreshold, posExpectedFeatureTrain, negExpectedFeatureTrain)

"""
active learning with random initialization and least confidence query strategy, keep a list of models 
"""
...
class active_learning:
	...
	def __init__(self, fold, rounds, fn, label):
		...
		### (1-weight)*score1+(weight)*score2
		self.m_selectWeight = 0.6

	def select_example(self, unlabeled_list):
		unlabeledIdScoreMap = {} ###unlabeledId:idscore
		unlabeledIdNum = len(unlabeled_list)

		## weightedCombineTwoModels
		for unlabeledIdIndex in range(unlabeledIdNum):
			unlabeledId = unlabeled_list[unlabeledIdIndex]

			idScore1 = self.getScore4Example(unlabeledId, self.m_modelList[0])
			# unlabeledIdScoreMap[unlabeledId] = idScore1

			idScore2 = self.getScore4Example(unlabeledId, self.m_modelList[1])

			# if idScore1 != idScore2:
			# 	print("idScore", idScore1, idScore2)

			unlabeledIdScoreMap[unlabeledId] = (1-self.m_selectWeight)*idScore1+self.m_selectWeight*idScore2
			# exit()
	...
	def getScore4Example(self, exID, model):
		labelPredictProb = model.predict_proba(self.fn[exID].reshape(1, -1))[0]
	...
	def run_CV(self):
		...
		for foldIndex in range(foldNum):
			...
			while queryIter < rounds:
				...
				if len(self.m_modelList) < 2:
					# print(self.clf.coef_)
					self.m_modelList = [deepcopy(self.clf), deepcopy(self.clf)]
					# print(self.m_modelList[0].coef_)
					# print(self.m_modelList[1].coef_)
					# self.m_modelList.append(clone(self.clf))
				else:
					self.m_modelList[1] = deepcopy(self.clf)
				...
				if len(self.m_modelList) < 2:
					...
					self.m_modelList[0] = deepcopy(self.clf)

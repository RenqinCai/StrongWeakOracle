...
class active_learning:
	...
	def run_CV(self, dataDir, sourceName, targetName):
		...
		self.m_clf = RFC(n_estimators=100, criterion='entropy', random_state=3)
...
def readSensorData(sensorLocation):
	if sensorLocation == "rice":
		input1 = np.genfromtxt("../../../dataset/sensorType/rice_hour_sdh", delimiter=",")
		fd1 = input1[:, 0:-1]
		target_fd = fd1
		target_label = input1[:,-1]

		featureMatrix = target_fd
		labelList = target_label

		return featureMatrix, labelList
	
	if sensorLocation == "sdh":
		...
		featureMatrix = source_fd
		labelList = sourceLabel
...
if __name__ == "__main__":
	...
	sourceFeatureMatrix, sourceLabelList = readSensorData("sdh")
	...
	targetFeatureMatrix, targetLabelList = readSensorData("rice")
	...
	dataDir = "../../../dataset/sensorType/sdh_rice/"

	al.run_CV(dataDir, "sdh", "rice")

"""
proactive learning Margin with random initialization, train a linear auditor to judge whether transfer learning is correct. reduce the feature dimension for auditor to 20 
"""
...
modelName = "Margin_linearAuditor_10FeatureAuditor_"+dataName
...
class _ProactiveLearning:
	...
	def __init__(self, fold, rounds, featureMatrix, label, transferLabel):
		...
		self.m_targetNameFeature4Auditor = np.array(featureMatrix)
	...
	def get_transfer_flag(self, transferFeatureList, transferFlagList, exId, judgeRound):
		...
		transferFlag = self.get_judgeClassifier_prob(self.m_judgeClassifier.coef_, self.m_targetNameFeature4Auditor[exId].reshape(1, -1), CB, judgeRound)
	...
	def PCAFeature(self, reducedDim):
		...
		self.m_targetNameFeature4Auditor = pca.fit_transform(self.m_targetNameFeature)
	...
	def run_CV(self):
		...
		for foldIndex in range(foldNum):
			...
			auditorNameFeatureTest = self.m_targetNameFeature4Auditor[test]
			...
			while activeLabelNum < rounds:
				...
				if transferLabelFlag:
					...
					if transferLabel == exLabel:
						...
						transferFeatureList.append(self.m_targetNameFeature4Auditor[exId])
					else:
					...
					auditorAcc = self.getAuditorMetric(transferFeatureList, transferFlagList, auditorNameFeatureTest, transferLabelTest, targetLabelTest)

"""
proactive learning with random initialization margin, linear judge classifier to judge whether transfer learning is correct. active learning for auditor only
"""
...
modelName = "Margin4Auditor_linearAuditor"+dataName
...
class _ProactiveLearning:
	...
	def pretrainSelectInit(self, train):
		...
		for trainIndex in range(len(train)):
			if self.m_targetLabel[train[trainIndex]] == self.m_transferLabel[train[trainIndex]]:
				if self.m_targetLabel[train[trainIndex]] == 1.0:
					posTrain.append(train[trainIndex])
			else:
				if self.m_targetLabel[train[trainIndex]] == 0.0:

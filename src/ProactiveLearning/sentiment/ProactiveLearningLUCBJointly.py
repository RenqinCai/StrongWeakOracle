"""
proactive learning with random initialization, judge classifier to judge whether transfer learning is correct. Active learn for both models jointly
"""
...
modelName = "LUCB_jointly_"+dataName
...
class _ProactiveLearning:
	...
	def select_example(self, unlabeled_list, auditorFlag):
		...
		for unlabeledIdIndex in range(unlabeledIdNum):
			...
			selectCoefDiff = np.abs(np.dot(self.m_clf.coef_, self.m_targetNameFeature[unlabeledId]))

			selectLCB = selectCoefDiff-2*0.002*selectCB
			selectIdScore = 1-selectLCB

			auditorIdScore = 0.0

			if auditorFlag:
				auditorCB = self.get_judge_confidence_bound(unlabeledId)
				auditorCoefDiff = np.abs(np.dot(self.m_judgeClassifier.coef_, self.m_targetNameFeature[unlabeledId]))
				auditorLCB = auditorCoefDiff-2*0.002*auditorCB
				auditorIdScore = 1-auditorLCB

			unlabeledIdScoreMap[unlabeledId] = selectIdScore+auditorIdScore
	...
	def run_CV(self):
		...
		for foldIndex in range(foldNum):
			...
			for initID in initExList:
				transferFeatureList.append(self.m_targetNameFeature[initID])
				if self.m_targetLabel[initID] == self.m_transferLabel[initID]:
					transferFlagList.append(1.0)
				else:
					transferFlagList.append(0.0)
			print(transferFlagList)
			...
			while activeLabelNum < rounds:
				...
				if len(set(transferFlagList)) == 2.0:
					self.m_judgeClassifier.fit(np.array(transferFeatureList), np.array(transferFlagList))
					auditorFlag = True

				self.m_clf.fit(targetNameFeatureIter, targetLabelIter) 

				exId = self.select_example(unlabeledExList, auditorFlag) 

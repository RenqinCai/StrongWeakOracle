"""
proactive learning with random initialization, an auditor to judge whether transfer learning is correct. Reuse previous wrong transferred instances, in descending order
"""
...
class _ProactiveLearning:
	...
	def get_transfer_flag(self, transferFeatureList, transferFlagList, transferRefScore, exId):
		...
		if auditorPosProb > transferRefScore:
			transferFlag = True
		else:
			transferFlag = False
	...
	def getTransferRefScore(self, transferFeatureList, transferFlagList, targetNameFeatureValid, targetLabelValid):
		transferRefScore = 0.0

		targetLabelValidPreds = self.m_clf.predict(targetNameFeatureValid)
		if len(np.unique(transferFlagList)) > 1:
			self.m_auditor.fit(np.array(transferFeatureList), np.array(transferFlagList))
		else:
			return transferRefScore

		targetAuditorValidPredProbs = self.m_auditor.predict_proba(targetNameFeatureValid)
		# print(targetAuditorValidPredProbs.shape)
		acc = accuracy_score(targetLabelValid, targetLabelValidPreds)
		print("valid acc", acc)
		validPredProbMap = {}  ### exID: predProb

		for validEx, auditorValidPredProb in enumerate(targetAuditorValidPredProbs):
			validPredProbMap.setdefault(validEx, auditorValidPredProb[1])

		# sortedValidExList = sorted(validPredProbMap, key=validPredProbMap.__getitem__, reverse=True)
		sortedValidExList = sorted(validPredProbMap, key=validPredProbMap.__getitem__)
		
		validNum = len(sortedValidExList)
		for validIndex in range(1, validNum):
			validEx = sortedValidExList[validIndex]
			accRef = 0.0
			correctRef = 0.0
			validExNum = 0.0

			for postValidIndex in range(validIndex, validNum):
				postValidEx = sortedValidExList[postValidIndex]
				validPredLabel = targetLabelValidPreds[postValidEx]
				if validPredLabel == targetLabelValid[postValidEx]:
					correctRef += 1.0
				validExNum += 1.0
			accRef = correctRef/validExNum
			print("accRef", accRef)

			if accRef >= acc:
				transferRefScore = targetAuditorValidPredProbs[validEx][1]
				return transferRefScore
				
		transferRefScore = targetAuditorValidPredProbs[sortedValidExList[-1]][1]
		return transferRefScore
	...
	def addExtraWeakLabels(self, transferFeatureList, transferFlagList, targetNameFeatureValid, targetLabelValid, targetNameFeatureTest, transferLabelTest, targetLabelTest, iteration):
		...
		print("~~~~~~~~~~~~~~~~~~%d~~~~~~~~~~~~~~~~~~~~~~"%iteration)
		transferRefScore = self.getTransferRefScore(transferFeatureList, transferFlagList, targetNameFeatureValid, targetLabelValid)
		print("transferRefScore", transferRefScore)
		for extraIndex in range(extraNum):
			...
			transferFlag, predLabel = self.get_transfer_flag(transferFeatureList, transferFlagList, transferRefScore, extraID)
	...
	def run_CV(self):
		...
		for foldIndex in range(foldNum):
			...
			trainData, valid = train_test_split(train, random_state=3, test_size=0.2)
			...
			while activeLabelNum < rounds:
				...
				extraAcc = self.addExtraWeakLabels(transferFeatureList, transferFlagList, targetNameFeatureValid, targetLabelValid, targetNameFeatureTest, transferLabelTest, targetLabelTest, queryIter)
...
if __name__ == "__main__":
	...
	modelName = "AAWS_validation_v2_"+dataName

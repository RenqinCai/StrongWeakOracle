...
class _ProactiveLearning:
	...
	def get_transfer_flag(self, transferFeatureList, transferFlagList, exId, iteration):
		...
		transferFlag = False
		if auditorPosProb > 1/(1+np.exp(-iteration/100)):
			transferFlag = True
		else:
			transferFlag = False
	...
	def addExtraWeakLabels(self, transferFeatureList, transferFlagList, targetNameFeatureTest, transferLabelTest, targetLabelTest, iteration):
		...
		for extraIndex in range(extraNum):
			...
			transferFlag, predLabel = self.get_transfer_flag(transferFeatureList, transferFlagList, extraID, iteration)
	...
	def run_CV(self):
		...
		for foldIndex in range(foldNum):
			...
			while activeLabelNum < rounds:
				...
				extraAcc = self.addExtraWeakLabels(transferFeatureList, transferFlagList, targetNameFeatureTest, transferLabelTest, targetLabelTest, queryIter)
...
if __name__ == "__main__":
	...
	modelName = "AAWS_dynamic_"+dataName

"""
proactive learning with random initialization, actively select instances from the unlabeled instances and are not included in the weak transferred list. Reuse previous wrong transferred instances and we use slab threshold to filter instances. We update the feature representation with labeled instances by strong oracle.
"""
...
class _ProactiveLearning:
	...
	def select_example(self, unlabeled_list, poisonScoreMap):
		...
		for unlabeledIdIndex in range(unlabeledIdNum):
			...
			## fine tune
			scoreWeightClassifier = 1.0

			poisonScore = poisonScoreMap[unlabeledId]
			unlabeledIdScoreMap[unlabeledId] = scoreWeightClassifier*idScoreClassifier + (1.0 - scoreWeightClassifier)*poisonScore
	...
	def generateCleanData(self, train, slabThreshold):
		...
		posExpectedFeatureTrain = [0.0 for i in range(len(self.m_targetNameFeature[0]))]
		negExpectedFeatureTrain = [0.0 for i in range(len(self.m_targetNameFeature[0]))]
		...
		for trainIndex in range(sampledTrainNum):
			...
			feature = self.m_targetNameFeature[trainID]

			trueLabel = self.m_targetLabel[trainID]
			transferLabel = self.m_transferLabel[trainID]
			...
			if trainID in self.m_strongLabeledIDList:
				if trueLabel == 1:
					posExpectedFeatureTrain += feature
					posLabelNum += 1.0
				else:
		...
		for trainIndex in range(sampledTrainNum):
			...
			featureTrain = self.m_targetNameFeature[trainID]
			transferLabel = self.m_transferLabel[trainID]
			trueLabel = self.m_targetLabel[trainID]
			...
			if poisonScore < slabDisThreshold:
				cleanFeatureTrain.append(self.m_targetNameFeature[trainID])
				cleanLabelTrain.append(transferLabel)

				cleanTrain.append(trainID)

				if transferLabel == trueLabel:
					correctCleanNum += 1.0

			poisonScoreMap[trainID] = poisonScore
		...
		return cleanFeatureTrain, cleanLabelTrain, cleanTrain, poisonScoreMap
		
	def addExtraWeakLabels(self, iteration):
		...
		weakLabelNum = len(weakLabel)
		if weakLabelNum == 0.0:
			print("weak label num", weakLabelNum)
			trainNameFeature = strongNameFeature
			trainLabel = strongLabel
			...
			print("weak label num", weakLabelNum, "extra correct labels ratio", correctTransferLabelNum*1.0/weakLabelNum)
	...
	def run_CV(self):
		...
		totalWeakLabelPrecisionList = []
		totalWeakLabelRecallList = []
		totalWeakLabelAccList = []
		totalWeakLabelNumList = []
		
		totalAuditorPrecisionList = []
		totalAuditorRecallList = []
		totalAuditorAccList = []

		slabThreshold = 0.0188
		for foldIndex in range(foldNum):
			...
			cleanFeatureTrain, cleanTransferLabelTrain, cleanTrain, poisonScoreMap = self.generateCleanData(train, slabThreshold)
			weakIDList = cleanTrain

			for exId in initExList:
				if exId in weakIDList:
					print("error exID in weakIDlist", exId)
					weakIDList.remove(exId)

			while activeLabelNum < rounds:

				exId = self.select_example(unlabeledExList, poisonScoreMap) 
				...
				if exId in weakIDList:
					print("error weakIDList 2", exId)
					weakIDList.remove(exId)

				weakNameFeature = self.m_targetNameFeature[weakIDList]
				weakLabel = self.m_transferLabel[weakIDList]
				...
				if weakLabelNum == 0.0:
					...
					print("trainAcc", trainAcc)
					extraAccList.append(trainAcc)
				else:
					# correctTransferLabelNum = np.sum(weakLabel==self.m_targetLabel[weakIDList])
					### weakLabelNum = TP + FP
					weakLabelTP = np.sum(weakLabel==self.m_targetLabel[weakIDList])
					weakLabelPrecision = weakLabelTP*1.0/weakLabelNum

					###totalCorrectWeakLabel = TP+FN
					totalCorrectWeakLabel = np.sum(self.m_transferLabel[unlabeledExList] == self.m_targetLabel[unlabeledExList])
					weakLabelRecall = weakLabelTP*1.0/(totalCorrectWeakLabel)

					#### len(unlabeledExList) = TP+FP+FN+TN
					weakLabelUntransferredNum = len(unlabeledExList)-weakLabelNum
					weakLabelTN = weakLabelUntransferredNum-(totalCorrectWeakLabel-weakLabelTP)
					weakLabelAcc = (weakLabelTP+weakLabelTN)/len(unlabeledExList)
					
					print("weak label num", weakLabelNum, "extra correct labels ratio", correctTransferLabelNum*1.0/weakLabelNum, weakLabelPrecision, weakLabelRecall, weakLabelAcc)
					...
					weakLabelNumList.append(weakLabelNum)
					extraAccList.append(trainAcc)
					weakLabelPrecisionList.append(weakLabelPrecision)
					weakLabelRecallList.append(weakLabelRecall)
					weakLabelAccList.append(weakLabelAcc)
			...
			totalWeakLabelNumList.append(weakLabelNumList)
			totalWeakLabelPrecisionList.append(weakLabelPrecisionList)
			totalWeakLabelRecallList.append(weakLabelRecallList)
			totalWeakLabelAccList.append(weakLabelAccList)
		...
		WeakLabelNumFile = modelVersion+"_weakLabelNum.txt"
		writeFile(totalWeakLabelNumList, WeakLabelNumFile)

		totalWeakLabelPrecisionFile = modelVersion+"_weakLabel_precision.txt" 
		writeFile(totalWeakLabelPrecisionList, totalWeakLabelPrecisionFile) 

		totalWeakLabelRecallFile = modelVersion+"_weakLabel_recall.txt" 
		writeFile(totalWeakLabelRecallList, totalWeakLabelRecallFile)

		totalWeakLabelAccFile = modelVersion+"_weakLabel_acc.txt" 
		writeFile(totalWeakLabelAccList, totalWeakLabelAccFile)
...
if __name__ == "__main__":
	...
	modelName = "AAWS_slab_updateFeature_weight1.0_"+dataName
	...
	al = _ProactiveLearning(fold, rounds, featureMatrix, labelArray, transferLabelArray, "20News", multipleClassFlag)

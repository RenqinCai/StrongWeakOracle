...
class _ProactiveLearning:
	...
	def __init__(self, fold, rounds, featureMatrix, label, transferLabel0, transferLabel1, category, multipleClass):
	...
	def get_transfer_flag(self, transferFeatureList, transferFlagList0, transferFlagList1, exId, judgeRound):
		...
		judgeThresholdSame = 0.70
		judgeThresholdDiff = 0.80
	...
	def getAuditorMetric(self, transferFeatureList, transferFlagList0, transferFlagList1, transferFeatureTest, transferLabelTest0, transferLabelTest1, targetLabelTest):
		acc0 = 0.0
		acc1 = 0.0
		if len(np.unique(transferFlagList0)) > 1:
			self.m_auditor0.fit(np.array(transferFeatureList), np.array(transferFlagList0))
			auditorLabelTest0 = (transferLabelTest0==targetLabelTest)
			predictAuditorLabelTest0 = self.m_auditor0.predict(transferFeatureTest)
			acc0 = accuracy_score(auditorLabelTest0, predictAuditorLabelTest0)

		if len(np.unique(transferFlagList1)) > 1:
			self.m_auditor1.fit(np.array(transferFeatureList), np.array(transferFlagList1))
			auditorLabelTest1 = (transferLabelTest1==targetLabelTest)
			predictAuditorLabelTest1 = self.m_auditor1.predict(transferFeatureTest)
			acc1 = accuracy_score(auditorLabelTest1, predictAuditorLabelTest1)
	...
	def run_CV(self):
		...
		totalAuditorAccList0 = []
		totalAuditorAccList1 = []
		...
		for foldIndex in range(foldNum):
			...
			transferLabelTest0 = self.m_transferLabel0[test]
			transferLabelTest1 = self.m_transferLabel1[test]
			...
			while activeLabelNum < rounds:
				...
				if transferLabelFlag:
					...
					auditorAcc0, auditorAcc1 = self.getAuditorMetric(transferFeatureList, transferFlagList0, transferFlagList1, targetNameFeatureTest, transferLabelTest0, transferLabelTest1, targetLabelTest)
					print("auditorAcc0", auditorAcc0, "auditorAcc1", auditorAcc1)

					auditorAccList0.append(auditorAcc0)
					auditorAccList1.append(auditorAcc1)
			...
			totalAuditorAccList0.append(auditorAccList0)
			totalAuditorAccList1.append(auditorAccList1)
		...
		print("correct transfer num\t", np.mean(correctTransferLabelNumList), np.sqrt(np.var(correctTransferLabelNumList)))

		AuditorAccFile0 = modelVersion+"_auditor0_acc.txt"
		writeFile(totalAuditorAccList0, AuditorAccFile0)

		AuditorAccFile1 = modelVersion+"_auditor1_acc.txt"
		writeFile(totalAuditorAccList1, AuditorAccFile1)
...
if __name__ == "__main__":
	...
	al = _ProactiveLearning(fold, rounds, featureMatrix, labelArray, transferLabelArray0, transferLabelArray1, "sentiment_electronics", multipleClassFlag)
